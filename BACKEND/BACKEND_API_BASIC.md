...
5. 若未找到：
   - 记录 warning 日志；
   - 返回 `{"success": False, "message": "科室不存在"}`（**具体 message 以代码实际为准，当前文件中使用中文提示**），HTTP 404。
6. 若找到：
   - 组装返回数据（源码中关键字段）：
     - `id`
     - `name`
...
1. **分页与搜索**
   - `GET /api/departments`、`GET /api/medicines` 增加分页参数、模糊查询（name 关键字等）。
2. **统一返回结构**
   - 为所有接口采用统一的结构，如：
     ```json
     ...
     ```
3. **软删除**
   - 为科室、药品增加 `is_active` 字段，实现软删除，便于审计和恢复。
4. **权限控制**
   - 将删除和更新操作限制为特定角色（例如管理员）。
