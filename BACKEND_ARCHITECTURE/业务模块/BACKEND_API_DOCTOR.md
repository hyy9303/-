...
所有接口都直接挂在 `doctor_bp` 上，无额外 URL 前缀（路径已经包含 `/api`）。
...
1. **权限控制**
   ...
   - 可在后续引入角色（管理员、科室主任等），限制谁可以修改/删除医生信息。

2. **软删除支持**
   - 现在是物理删除（`DELETE FROM doctors`）；
   - 若对审计要求更高，可在 `DOCTORS` 表中增加 `is_active` 字段，改为“软删除”。
...
4. **与统计模块联动**
   - 统计接口中可基于医生维度（就诊数量、开具处方数量等）做更多分析；
   - `pendingCount` 的设计已经为“医生当前工作负载”提供了基础数据。
